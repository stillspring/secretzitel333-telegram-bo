...
class Config:
    ...
    def __init__(self):
        ...
        self.OTHER_RESPONSES: List[str] = self._parse_other_responses(other_responses_str)
        
        # Bot behavior configuration
        self.CASE_SENSITIVE: bool = os.getenv('CASE_SENSITIVE', 'false').lower() == 'true'
        
        # Logging configuration
        log_level = os.getenv('LOG_LEVEL', 'INFO').upper()
        self.LOG_LEVEL: str = log_level if log_level in ['DEBUG', 'INFO', 'WARNING', 'ERROR'] else 'INFO'
    ...
    def _parse_other_responses(self, responses_str: str) -> List[str]:
        ...
        try:
            ...
            if isinstance(responses, list) and all(isinstance(r, str) for r in responses):
                return responses
            else:
                logger.warning("OTHER_RESPONSES is not a valid JSON array of strings")
        ...
        return [responses_str.strip()] if responses_str.strip() else self._get_default_responses()
